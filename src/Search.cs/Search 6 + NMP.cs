...
public class Search_6_NMP : Search
{
    public override string ToString() { return "Search_6_NMP"; }
    ...
    public override Move Think(Board board, long timeControl)
    {
        ...
        for (; depth<10; depth++)
        {            
            score = -negaMax(-CHECKMATE, CHECKMATE, depth, true);
            if (watch.ElapsedMilliseconds > timeControl) break;
        }

        //Console.WriteLine("+NMP "+depth);
        watch.Stop();

        return globalBestMove;
    }

    private int negaMax(int alpha, int beta, int depth, bool doNull)
    {
        ...
        // NMP
        int score;
        if (!board.isInCheck && depth>=2 && doNull)
        {
            board.makeNullMove();
            score = -negaMax(-beta, 1-beta, depth - 3, false);
            board.undoNullMove();
        ...
        foreach (Move move in moves)
        {
            ...
            board.makeMove(move);
            score = -negaMax(-beta, -alpha, depth-1, doNull);
            board.undoMove(move);
