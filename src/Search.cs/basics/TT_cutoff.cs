...
public class TT_cutoff : Search
{
    ...
    public int Search(int alpha, int beta, int depth)
    {
        ...
        bool ttHit = entry.zobristKey == board.zobristKey;
        if (ttHit && !isRoot && depth<=entry.depth && (
            entry.flag==2 && entry.score >= beta ||
            entry.flag==0 && entry.score < alpha ||
            entry.flag==1 // && nonPV
        ))
            return entry.score;
        ...
        byte flag = (bestScore < alpha) ? (byte) 0 : 
                    (bestScore >= beta) ? (byte) 2 :
                                          (byte) 1;
