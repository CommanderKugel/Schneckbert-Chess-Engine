...
//
// NEW STUFF:
// initializing best score for Move ordering a lot smaller
// allow History heuristic in Qsearch (quiet moves allowed if in check)
// 
// 
...
public class Search_17_MOFix : Search
{
    public override string ToString() { return "17_MOFix"; }
    ...
    private int qSearch(int alpha, int beta, int depth)
    {
        ...
        // if not in check: only captures allowed, 
        if (!board.isInCheck)
        {
            for (int i=0; i<moveScores.Length; i++)
            {   
        ...
        // if in check: also quiet moves allowed -> order by history heuristic
        else 
        {
            int us = board.isWhiteToMove ? 1 : 0;
            Move move;
