public static class PestoTables
{
    private static readonly int[] mg_pawn_table = { 
        0, 0, 0, 0, 0, 0, 0, 0, 
        -22, 38, 24, -15, -23, -20, -1, -35, 
        -22, 33, 3, 3, -10, -4, -4, -26, 
        -25, 10, 6, 17, 12, -5, -2, -27, 
        -23, 17, 12, 23, 21, 6, 13, -14, 
        -20, 25, 56, 65, 31, 26, 7, -6, 
        -11, 34, 126, 68, 95, 61, 134, 98,
        0, 0, 0, 0, 0, 0, 0, 0 
    };

    private static readonly int[] eg_pawn_table = { 
        0, 0, 0, 0, 0, 0, 0, 0, 
        -7, 2, 0, 13, 10, 8, 8, 13,
        -8, -1, -5, 0, 1, -6, 7, 4, 
        -1, 3, -8, -7, -7, -3, 9, 13, 
        17, 17, 4, -2, 5, 13, 24, 32, 
        84, 82, 53, 56, 67, 85, 100, 94, 
        187, 165, 132, 147, 134, 158, 173, 178, 
        0, 0, 0, 0, 0, 0, 0, 0 
    };

    private static readonly int[] mg_knight_table = { 
        -23, -19, -28, -17, -33, -58, -21, -105, 
        -19, -14, 18, -1, -3, -12, -53, -29, 
        -16, 25, 17, 19, 10, 12, -9, -23, 
        -8, 21, 19, 28, 13, 16, 4, -13, 
        22, 18, 69, 37, 53, 19, 17, -9, 
        44, 73, 129, 84, 65, 37, 60, -47, 
        -17, 7, 62, 23, 36, 72, -41, -73, 
        -107, -15, -97, 61, -49, -34, -89, -167 
    };

    private static readonly int[] eg_knight_table = { 
        -64, -50, -18, -22, -15, -23, -51, -29, 
        -44, -23, -20, -2, -5, -10, -20, -42, 
        -22, -20, -3, 10, 15, -1, -3, -23, 
        -18, 4, 17, 16, 25, 16, -6, -18, 
        -18, 8, 11, 22, 22, 22, 3, -17, 
        -41, -19, -9, -1, 9, 10, -20, -24,
        -52, -24, -25, -9, -2, -25, -8, -25,
        -99, -63, -27, -31, -28, -13, -38, -58
    };

    private static readonly int[] mg_bishop_table = { 
        -21, -39, -12, -13, -21, -14, -3, -33, 
        1, 33, 21, 7, 0, 16, 15, 4, 
        10, 18, 27, 14, 15, 15, 15, 0,
        4, 10, 12, 34, 26, 13, 13, -6,
        -2, 7, 37, 37, 50, 19, 5, -4, 
        -2, 37, 50, 35, 40, 43, 37, -16, 
        -47, 18, 59, 30, -13, -18, 16, -26, 
        -8, 7, -42, -25, -37, -82, 4, -29 
    };

    private static readonly int[] eg_bishop_table = { 
        -17, -5, -16, -9, -5, -23, -9, -23, 
        -27, -15, -9, 4, -1, -7, -18, -14, 
        -15, -7, 3, 13, 10, 8, -3, -12, 
        -9, -3, 10, 7, 19, 13, 3, -6, 
        2, 3, 10, 14, 9, 12, 9, -3,
        4, 0, 6, -2, -1, 0, -8, 2, 
        -14, -4, -13, -3, -12, 7, -4, -8, 
        -24, -17, -9, -7, -8, -11, -21, -14
    };

    private static readonly int[] mg_rook_table = { 
        -26, -37, 7, 16, 17, 1, -13, -19, 
        -71, -6, 11, -1, -9, -20, -16, -44, 
        -33, -5, 0, 3, -17, -16, -25, -45, 
        -23, 6, -7, 9, -1, -12, -26, -36, 
        -20, -8, 35, 24, 26, 7, -11, -24, 
        16, 61, 45, 17, 36, 26, 19, -5,
        44, 26, 67, 80, 62, 58, 32, 27, 
        43, 31, 9, 63, 51, 32, 42, 32
    }; 

    private static readonly int[] eg_rook_table = {
        -20, 4, -13, -5, -1, 3, 2, -9, 
        -3, -11, -9, -9, 2, 0, -6, -6, 
        -16, -8, -12, -7, -1, -5, 0, -4, 
        -11, -8, -6, -5, 4, 8, 5, 3, 
        2, -1, 1, 2, 1, 13, 3, 4, 
        -3, -5, -3, 4, 5, 7, 7, 7, 
        3, 8, 3, -3, 11, 13, 13, 11, 
        5, 8, 12, 12, 15, 18, 10, 13
    };

    private static readonly int[] mg_queen_table = {
        -50, -31, -25, -15, 10, -9, -18, -1, 
        1, -3, 15, 8, 2, 11, -8, -35, 
        5, 14, 2, -5, -2, -11, 2, -14, 
        -3, 3, -4, -2, -10, -9, -26, -9, 
        1, -2, 17, -1, -16, -16, -27, -27, 
        57, 47, 56, 29, 8, 7, -17, -13,
        54, 28, 57, -16, 1, -5, -39, -24, 
        45, 43, 44, 59, 12, 29, 0, -28
    };

    private static readonly int[] eg_queen_table = { 
        -41, -20, -32, -5, -43, -22, -28, -33, 
        -32, -36, -23, -16, -16, -30, -23, -22, 
        5, 10, 17, 9, 6, 15, -27, -16, 
        23, 39, 34, 31, 47, 19, 28, -18, 
        36, 57, 40, 57, 45, 24, 22, 3,
        9, 19, 35, 47, 49, 9, 6, -20, 
        0, 30, 25, 58, 41, 32, 20, -17, 
        20, 10, 19, 27, 27, 22, 22, -9
    };

    private static readonly int[] mg_king_table = { 
        14, 24, -28, 8, -54, 12, 36, -15, 
        8, 9, -16, -43, -64, -8, 7, 1, 
        -27, -15, -30, -44, -46, -22, -14, -14, 
        -51, -33, -44, -46, -39, -27, -1, -49, 
        -36, -14, -25, -30, -27, -12, -20, -17, 
        -22, 22, 6, -20, -16, 2, 24, -9, 
        -29, -38, -4, -8, -7, -20, -1, 29, 
        13, 2, -34, -56, -15, 16, 23, -65
    };

    private static readonly int[] eg_king_table ={
        -43, -24, -14, -28, -11, -21, -34, -53, 
        -17, -5, 4, 14, 13, 4, -11, -27, 
        -9, 7, 16, 23, 21, 11, -3, -19, 
        -11, 9, 23, 27, 24, 21, -4, -18, 
        3, 26, 33, 26, 27, 24, 22, -8, 
        13, 44, 45, 20, 15, 23, 17, 10, 
        11, 23, 38, 17, 17, 14, 17, -12, 
        -17, 4, 15, -11, -18, -18, -35, -74
    }; 

    
    public static int[][] mgTables = new int[6][];
    public static int[][] egTables = new int[6][];

    public static void initTables()
    {
        int[] mg_value = { 82, 337, 365, 477, 1025,  20_000};
        int[] eg_value = { 94, 281, 297, 512,  936,  20_000};
        int[][] mg_table = { mg_pawn_table, mg_knight_table, mg_bishop_table, mg_rook_table, mg_queen_table, mg_king_table };
        int[][] eg_table = { eg_pawn_table, eg_knight_table, eg_bishop_table, eg_rook_table, eg_queen_table, eg_king_table };

        for (int piece=0; piece < 6; piece++)
        {
            mgTables[piece] = new int[64];
            egTables[piece] = new int[64];

            for (int sq=0; sq<64; sq++)
            {
                mgTables[piece][sq] = mg_value[piece] + mg_table[piece][sq];
                egTables[piece][sq] = eg_value[piece] + eg_table[piece][sq];
            }

        }
    }

}